    ...
    def _reset(self, data_path, save):
        if not save:
            ...
            if not os.path.exists(os.path.join(self.dir_name, self.model_path)):
                exit()
                # Model(input_dim=args.input_dim, hidden_dim=args.hidden_dim, latent_dim=args.dim,
                #    num_hops=args.hops, num_mlp_layers=args.mlps, dropout=args.dropout, **cfg['GAE']).cuda()
            dataset = load_json(data_path)
            if type(dataset)==str:
                dataset = dict(eval(dataset))
            try:
                input_dim = np.asarray(dataset[0]['module_operations']).shape[1]
            except:
                input_dim = np.asarray(dataset['0']['module_operations']).shape[1]
            self.model = Model(input_dim=input_dim, hidden_dim=128, latent_dim=args.dim, num_hops=5, num_mlp_layers=2, dropout=0, **cfg['GAE']).cuda()
            ...
            with torch.no_grad():
                ...
                for ind in tqdm(range(len(dataset))):
                    try:
                        ...
                        try:
                            adj = torch.Tensor(dataset[ind]['module_adjacency']).unsqueeze(0).cuda()
                            ops = torch.Tensor(dataset[ind]['module_operations']).unsqueeze(0).cuda()
                            adj, ops, prep_reverse = preprocessing(adj, ops, **cfg['prep'])
                            test_acc = dataset[ind]['test_accuracy']['val_accuracy']
                            valid_acc = dataset[ind]['validation_accuracy']['val_accuracy']
                            time = dataset[ind]['training_time']
                        except:
                            adj = torch.Tensor(dataset[ind]['module_adjacency']).unsqueeze(0).cuda()
                            ops = torch.Tensor(dataset[ind]['module_operations']).unsqueeze(0).cuda()
                            adj, ops, prep_reverse = preprocessing(adj, ops, **cfg['prep'])
                            test_acc = dataset[ind]['test_accuracy']
                            valid_acc = dataset[ind]['validation_accuracy']
                            time = dataset[ind]['training_time']
...
if __name__ == '__main__':
    ...
    env = Env('REINFORCE', args.seed, args.emb_path, args.model_path, cfg, data_path=args.data_path, save=args.saved_arch2vec)
