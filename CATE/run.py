...
def train(config):
    ...
    for epoch_idx in range(config.max_epoch):
        ...
        for batch_idx, batch_data in enumerate(trainSet):
            ...
            if (ticks >= config.check_point_min) and (ticks % config.check_point_freq == 0):
                ...
                if is_best:
                    ...
                    if config.dataset=='nds':
                        fname_ = "%s_%s_%s_latest.pth.tar" % (config.dataset, config.search_space, config.type)
                    elif config.dataset=='transnasbench101':
                        fname_ = "%s_%s_latest.pth.tar" % (config.dataset, config.task)
                    else:
                        fname_ = config.dataset + '_latest.pth.tar'
                    save_check_point({
                        'epoch': epoch_idx + 1,
                        'batch': batch_idx + 1,
                        'config': config,
        ...
        if config.save_each_epoch:
            ...
            if config.dataset=='nds':
                fname_ = "%s_%s_%s_checkpoint_Epoch_%s.pth.tar" % (config.dataset, config.search_space, config.type, str(epoch_idx+1))
            elif config.dataset=='transnasbench101':
                fname_ = "%s_%s_checkpoint_Epoch_%s.pth.tar" % (config.dataset, config.task, str(epoch_idx+1))
            else:
                fname_ = config.dataset + '_checkpoint_Epoch_%s.pth.tar' % (str(epoch_idx+1))
            save_check_point({
                'epoch': epoch_idx + 1,
                'batch': batch_idx + 1,
                'config': config,
