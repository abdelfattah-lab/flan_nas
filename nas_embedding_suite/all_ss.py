...
class AllSS:
    def __init__(self):
        ...
        for space, space_idx in self.ss_mapper.items():
            ...
            elif space in ["Amoeba_in", "DARTS_in", "DARTS_lr-wd_in", "ENAS_in", "NASNet_in", "PNAS_in"]:
                exec(f'self.ss_mapper_oprange[space] = (np.asarray(self.nds_in.get_adj_op(0, space="{space}")["normal_ops"]).shape[-1], space_idx)')
            else:
    ...
    def get_adj_op(self, idx, space, bin_space=False):
        if bin_space:
            if space in ["nb101", "nb201", "nb301", "tb101"]:
                ...
                adj_op_mat = self.nds_in.get_adj_op(idx, space=space)
                new_adj_op_mat = copy.deepcopy(adj_op_mat)
    ...
    def get_zcp(self, idx, space, task="class_scene", joint=None):
        if space in ["nb101", "nb201", "nb301", "tb101"]:
            ...
            zcp = self.nds_in.get_zcp(idx, space=space)
        else:
    ...
    def get_norm_w_d(self, idx, space="Amoeba"):
        if space in ["nb101", "nb201", "nb301", "tb101"]:
            ...
            return self.nds_in.get_norm_w_d(idx, space=space)
        else:
    ...
    def get_arch2vec(self, idx, space, joint=False):
        if joint:
            ...
            if space in ["nb101", "nb201", "nb301", "tb101"]:
                ...
                arch2vec = self.nds_in.get_arch2vec(idx, space=space)
            else:
    ...
    def get_cate(self, idx, space, joint=False):
        if joint:
            ...
            if space in ["nb101", "nb201", "nb301", "tb101"]:
                ...
                cate = self.nds_in.get_cate(idx, space=space)
            else:
    ...
    def get_valacc(self, idx, space):
        if space in ["nb101", "nb201", "nb301", "tb101"]:
            ...
            valacc = self.nds_in.get_valacc(idx, space=space)
        else:
    ...
    def get_max_oplen(self):
        ...
        for ssk in self.sskeys:
            ...
            elif ssk in ["Amoeba_in", "DARTS_in", "DARTS_lr-wd_in", "ENAS_in", "NASNet_in", "PNAS_in"]:
                oplens[ssk + "_n"] = len(self.nds_in.get_adj_op(0, space=ssk)["normal_ops"][0])
                oplens[ssk + "_r"] = len(self.nds_in.get_adj_op(0, space=ssk)["reduce_ops"][0])
            else:
