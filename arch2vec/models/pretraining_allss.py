...
def group_by_shape(X_adj, X_ops):
    grouped_adj = []
    grouped_ops = []
    ...
    for adj, ops in zip(X_adj, X_ops):
        if current_shape is None or current_shape != adj.shape:
            if current_adj_group:  # add the previous group to the list
                grouped_adj.append(current_adj_group)
                grouped_ops.append(current_ops_group)

            current_shape = adj.shape
            current_adj_group = [adj]
            current_ops_group = [ops]
        else:
            current_adj_group.append(adj)
            current_ops_group.append(ops)

    # Add the last group
    if current_adj_group:
        grouped_adj.append(current_adj_group)
        grouped_ops.append(current_ops_group)

    return grouped_adj, grouped_ops

def pretraining_gae(dataset, cfg, text_signature):
    """
    ...
    for epoch in range(0, epochs):
        ...
        a = time.time()
        combined = list(zip(X_adj_train, X_ops_train))
        ...
        for adj_group, ops_group in zip(X_adj_grouped, X_ops_grouped):
            ...
            random.shuffle(combined_group)
            adj_group[:], ops_group[:] = zip(*combined_group)
        
        X_adj_split = [torch.split(torch.stack(group), bs, dim=0) for group in X_adj_grouped]
        X_ops_split = [torch.split(torch.stack(group), bs, dim=0) for group in X_ops_grouped]
        # flatten the list of list
        X_adj_split = [item for sublist in X_adj_split for item in sublist]
        X_ops_split = [item for sublist in X_ops_split for item in sublist]
        combined_split = list(zip(X_adj_split, X_ops_split))
        random.shuffle(combined_split)
        X_adj_split, X_ops_split = zip(*combined_split)
        indices_split = torch.split(indices_train, bs, dim=0)
        print("Transformation time: {}".format(time.time() - a))
        loss_epoch = []
        Z = []
        ...
        print("Epoch time: {}".format(time.time() - bn))
        # Z = torch.cat(Z, dim=0)
        ...
        save_checkpoint_vae(model, optimizer, text_signature, epoch, sum(loss_epoch) / len(loss_epoch), args.latent_dim, args.name, args.dropout, args.seed)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', type=str, default='data/all_ss.json')
    ...
    text_signature = 'dim_%s_search_space_all_ss' % str(args.latent_dim)
    pretraining_gae(dataset, cfg, text_signature)
